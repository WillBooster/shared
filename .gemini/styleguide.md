...
- Never explain how WillBooster's in-house tools (e.g., `wb`, `wbfy`) work in the code comments or hand-written documents (e.g., `README.md`) of repositories that merely use them; e.g., do not note that `PORT` is unset because `wb` picks a free port.
