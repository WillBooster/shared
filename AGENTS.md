...
- Never explain how WillBooster's in-house tools (e.g., `wb`, `wbfy`) work in the comments or documents of repositories that merely use them (e.g., do not note that `PORT` is unset because `wb` picks a free port); developers already know their specifications, so document them only inside the tool's own package.
