...
1. Run `yarn start <directory_path_of_target_repo>` in your local `wbfy` repository checkout.
...
3. If any check fails, do one of the following:
   ...
   - If the failure comes from `wbfy`, fix your local `wbfy` repository checkout, commit and push the change, open a PR in the `wbfy` repository, then go back to step 1.
