...
- After making changes, as a rule run only what the change needs locally, `bun run verify` (type checking and linting; up to 10 minutes) and the relevant tests, and leave the full test suite (`bun run verify-full`; up to 1 hour) to the PR's CI. If PR CI does not run the full suite, run it locally. Fix errors, CI failures included, and re-run until they pass.
