...
- Once verified, commit and push to the current (non-main) branch, and create a PR via `gh` if none exists for the branch.
  ...
  - Current-state rule: an existing Requirements section records what the requester asked for and what was decided as it stands now; it is neither a description of the diff nor a log of how the request evolved. Keep its `required` lines as they are while the requester's instructions stand (with no instructions available, keep them unchanged). When a later instruction overrides or withdraws an earlier one, rewrite or remove the earlier line so that no two lines conflict, instead of appending a line beside it, and never mark when a line was added (e.g., "(added)"). Add a `chosen` line for a new decision, and replace or remove one when the decision it records changed. When the current design cannot be understood without the change of direction, state it in one sentence in Notes. A section that merely describes the implementation, or holds only the template's placeholder or a `required: not recorded` line, is not a record: write it afresh by the rule above.
  ...
  - Never drop or weaken a `required` line to fit what was implemented: when one cannot hold in the diff, keep it and say so in Notes. Only the requester's own later instruction, or a change in the caller contract a line records, changes such a line.
...
- Never declare another workspace package in `packages/wb/package.json`: every shared package declares `@willbooster/wb`, and `multi-semantic-release` aborts the whole release on the resulting cycle. `wb`'s imports of them resolve through the root workspace, as its `@willbooster/shared-lib-node` ones always have.
