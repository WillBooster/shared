  ...
  it('escapes vitest targets inside concurrently commands', async () => {
    ...
    expect(command).toBe(
      buildShellCommand([
        ...
        `YARN wb wait-on -t 600000 -i 2000 tcp:localhost:3000 && curl -s -o /dev/null -m 5 --retry 150 --retry-delay 2 --retry-all-errors http://localhost:3000 && sleep 2 && curl -s -o /dev/null -m 5 --retry 150 --retry-delay 2 --retry-all-errors http://localhost:3000 && ${buildShellCommand(
          [
            'YARN',
            'vitest',
            'run',
