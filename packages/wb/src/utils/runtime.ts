...
/**
 * Whether the directory carries a bun marker: a bun lockfile or a mise-pinned bun. Repositories
 * that gitignore bun.lock and have no packageManager field rely on the tool manifest. This is the
 * single signal list deciding bun-vs-yarn; `Project.usesBunPackageManager` and `usesBunRuntime`
 * share it — and both give an explicit non-Bun packageManager declaration precedence over it
 * (see isExplicitNonBunPackageManager) — so the two detections cannot drift.
 */
