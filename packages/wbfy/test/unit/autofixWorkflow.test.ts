...
test('a private repository gets the App-based apply workflow instead of autofix.ci', async () => {
  ...
  try {
    ...
    // A called workflow can only reduce the caller's token, so actions:read (artifact download)
    // and id-token:write (the broker's OIDC exchange) must both be granted here.
    ...
    // No secret at all: the App's private key lives only in the token broker, and the callee no
    // longer declares AUTOFIX_APP_PRIVATE_KEY (passing an undeclared secret fails at startup).
    expect(workflow.jobs.apply.secrets).toBeUndefined();
  } finally {
    await fs.promises.rm(dirPath, { recursive: true, force: true });
  }
});

// Callers generated by the secret-injecting template (wbfy 9.2.0) would fail at startup once the
// reusable workflow stops declaring the secret, so regeneration must strip the pass-through.
test('regeneration removes the retired AUTOFIX_APP_PRIVATE_KEY pass-through from an existing caller', async () => {
  const tempRootPath = path.join(process.cwd(), '.tmp');
  await fs.promises.mkdir(tempRootPath, { recursive: true });
  const dirPath = await fs.promises.mkdtemp(path.join(tempRootPath, 'wbfy-autofix-'));
  try {
    ...
    await fs.promises.writeFile(
      path.join(workflowsPath, 'autofix-apply.yml'),
      `name: Apply autofix
on:
  workflow_run:
    workflows: [Test]
...
`
    ...
    const workflow = loadYaml(content) as {
      permissions: Record<string, string>;
      jobs: { apply: { secrets?: Record<string, string> } };
    };
    expect(workflow.jobs.apply.secrets).toBeUndefined();
    expect(workflow.permissions).toEqual({ actions: 'read', contents: 'read', 'id-token': 'write' });
  } finally {
