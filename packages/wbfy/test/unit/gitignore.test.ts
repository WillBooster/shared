...
test('keeps repository ignore rules across the first and subsequent runs', async () => {
  ...
  try {
    ...
    expect(checkIgnore('  # Project-specific settings')).toBe(0);
    expect(checkIgnore('see # Project-specific settings notes')).toBe(0);
...
test('preserves an escaped trailing space in the final imported rule', async () => {
  ...
  try {
    ...
    expect(checkIgnore()).toBe(0);
    await generatePrettierignore(config);
    expect(fs.readFileSync(path.join(tempDirPath, '.prettierignore'), 'utf8')).toContain('foo\\ \n');
    await generateGitignore(config, config);
    expect(checkIgnore()).toBe(0);
  } finally {
    fs.rmSync(tempDirPath, { force: true, recursive: true });
...
test('mirrors rules following double-hash comments into Prettier ignore', async () => {
  const tempDirPath = await fs.promises.realpath(fs.mkdtempSync(path.join(os.tmpdir(), 'wbfy-gitignore-')));
  try {
    fs.writeFileSync(path.join(tempDirPath, '.gitignore'), '## Generated by tool\nbuild/\n');
    const config = createConfig({ dirPath: tempDirPath, isRoot: true });
    await generateGitignore(config, config);
    await generatePrettierignore(config);
    expect(fs.readFileSync(path.join(tempDirPath, '.prettierignore'), 'utf8')).toContain('\nbuild/\n');
...
test('retains rules prepended to a managed ignore file', async () => {
  ...
  try {
    ...
    expect(content).toContain('# Project-specific settings (tail)\nlocal-cache/\n');
    expect(content.match(/# Generated by wbfy/g)).toHaveLength(1);
  } finally {
    fs.rmSync(tempDirPath, { force: true, recursive: true });
  }
});

test('imports every rule around a foreign generated banner', async () => {
  const tempDirPath = await fs.promises.realpath(fs.mkdtempSync(path.join(os.tmpdir(), 'wbfy-gitignore-')));
  try {
    const filePath = path.join(tempDirPath, '.gitignore');
    fs.writeFileSync(
    ...
    await generateGitignore(config, config);
    const content = fs.readFileSync(filePath, 'utf8');
    expect(content).toContain('keep-head/\n');
    expect(content).toContain('keep-middle/\n');
    expect(content).toContain('keep-tail/\n');
  } finally {
    fs.rmSync(tempDirPath, { force: true, recursive: true });
  }
});
