...
- After making changes, as a rule run only what the change needs locally, \`${packageManager} run verify\` (type checking and linting; up to 10 minutes) and the relevant tests, and leave the full test suite (\`${packageManager} run verify-full\`; up to 1 hour) to the PR's CI. Fix errors, CI failures included, and re-run until they pass.
  - Run relevant tests with \`${packageManager} wb test test/unit/example.test.ts --grep 'case name'\` (omit \`--grep\` for the whole file; use \`-w packages/example\` in a monorepo). \`${packageManager} run verify-full <test-path> --grep 'case name'\` combines verification with selected tests.
