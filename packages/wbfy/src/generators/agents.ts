...
- After making changes, run \`${packageManager} verify\` (type checking and linting; takes up to 10 minutes), or \`${packageManager} verify-full\` (all tests; takes up to 1 hour) if you changed runtime behavior or tests. Fix errors and re-run until it passes.
  - Agent shells terminate tracked commands (including background ones) after about 10 minutes, so run commands that can exceed that (e.g., \`${packageManager} verify-full\`) detached via nohup, in a shell call that returns immediately (a bare \`&\` inside a waiting shell is still tracked and killed): \`mkdir -p .tmp; nohup ${packageManager} verify-full > .tmp/verify-full.log 2>&1 & echo $! > .tmp/verify-full.pid\` (\`;\` after \`mkdir\`, not \`&&\`, which would background the \`mkdir\` and race the PID-file write). Then poll from separate short calls, each capped well under 10 minutes (e.g., \`for i in {1..15}; do kill -0 "$(cat .tmp/verify-full.pid)" 2>/dev/null || break; sleep 30; done\`), repeating until the process exits, and read the log.
