...
- After making changes, run \`${packageManager} verify\` (type checking and linting; takes up to 10 minutes), or \`${packageManager} verify-full\` (all tests; takes up to 1 hour) if you changed runtime behavior or tests. Fix errors and re-run until it passes.
  - Agent shells may terminate tracked commands (including background ones) at time limits, often minutes, so run commands that can exceed your shell-call timeout (e.g., \`${packageManager} verify\` and \`${packageManager} verify-full\`) detached via nohup, from a shell call that returns immediately (a bare \`&\` inside a waiting shell is still tracked and killed): \`mkdir -p .tmp; nohup sh -c '${packageManager} verify-full; echo $? > .tmp/verify-full.exit' > .tmp/verify-full.log 2>&1 &\` (\`;\` after \`mkdir\`, not \`&&\`, which would background the \`mkdir\`). Poll from separate calls, each shorter than your shell-call timeout: \`for i in {1..3}; do test -f .tmp/verify-full.exit && break; sleep 20; done; cat .tmp/verify-full.exit 2>/dev/null || echo still running\`. Repeat until the exit file appears (its content is the exit code), then read the log; if it never appears while the log stops growing, the run itself died.
