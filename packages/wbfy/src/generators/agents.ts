...
- After making changes, run \`${packageManager} verify\` (type checking and linting; takes up to 10 minutes), or \`${packageManager} verify-full\` (all tests; takes up to 1 hour) if you changed runtime behavior or tests. Fix errors and re-run until it passes.
  - Agent shells terminate tracked commands (including background ones) after about 10 minutes, so run commands that can exceed that (e.g., \`${packageManager} verify-full\`) detached via nohup: \`mkdir -p .tmp && nohup ${packageManager} verify-full > .tmp/verify-full.log 2>&1 & echo $! > .tmp/verify-full.pid\`, then wait with \`until ! kill -0 "$(cat .tmp/verify-full.pid)" 2>/dev/null; do sleep 30; done\` and read the log.
