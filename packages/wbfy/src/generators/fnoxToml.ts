...
export const FNOX_AGE_PRINCIPALS = [
  ...
  },
  {
    ...
    repositoryScope: {
      repositories: ['WillBooster/exercode', 'WillBooster/judge'],
    },
  },
  {
    name: 'ayame',
    ...
    repositoryScope: {
      repositories: ['WillBooster/chofu-walking', 'WillBooster/exercode', 'WillBooster/judge'],
    },
  },
...
function replaceAgeRecipients(content: string, ageRecipients: readonly FnoxAgeRecipient[]): string {
  ...
  // because the negated class [^\]] (unlike `.`) spans newlines. Any other spelling falls through
  // to the caller's re-parse validation, which fails the run safely with instructions instead of
  // corrupting the file.
  const lines = content.split('\n');
  const headerIndex = lines.findIndex((line) => /^\s*\[providers\.age\]\s*(?:#.*)?$/u.test(line));
  if (headerIndex !== -1) {
