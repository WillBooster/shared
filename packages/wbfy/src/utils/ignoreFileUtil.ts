...
export const ignoreFileUtil = {
  ...
  isManaged(content: string): boolean {
    return headUserContentHeaderRegex.test(content) && content.includes(separatorPrefix);
  },
  ...
  getHeadUserContent(content: string): string {
    const index = content.indexOf(separatorPrefix);
    if (this.isManaged(content) && index !== -1) {
      return content
